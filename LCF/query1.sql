...
CREATE OR REPLACE VIEW AREA_COUNT_SALES AS
SELECT C.area, COUNT(A.id) TOTAL_CUSTOMERS, TO_CHAR(SUM(I.totalPrice), '999,999.99') TOTAL_SALES
FROM Customers C, Pets P, Appointments A, Invoices I
WHERE P.ownerIc = C.ic AND A.petId = P.id AND I.appointmentId = A.id
GROUP BY C.area
ORDER BY TOTAL_SALES DESC;

SELECT * FROM AREA_COUNT_SALES;
